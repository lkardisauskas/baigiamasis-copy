...
app.get("/", async (req, res) => {
  ...
  try {
    ...
    console.log("Data from GuestList collection:", guestDb); // Log the data to the console

    res.json(guestDb); // Send the data as the response
  } catch (err) {
...
app.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    const result = await client
      .db("Events")
      .collection("GuestList")
      .updateOne({ _id: new mongodb.ObjectId(id) }, { $set: updateFields });

    if (result.modifiedCount === 1) {
      res.status(200).send(`Guest with ID ${id} updated successfully`);
    } else {
      res.status(404).send("Guest not found");
    }
  } catch (error) {
    console.error("Error updating guest:", error);
    res.status(500).send("Failed to update guest. Please try again.");
  } finally {
    await client.close();
  }
});
